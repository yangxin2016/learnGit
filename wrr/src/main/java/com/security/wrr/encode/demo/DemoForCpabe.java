package com.security.wrr.encode.demo;
import com.security.wrr.encode.cpabe.Cpabe;
...
public class DemoForCpabe {
	...
	static String pubfile = "D:/file_dir/pub_key";
	static String mskfile = "D:/file_dir/master_key";
	static String prvfile = "D:/file_dir/prv_key";

	static String inputfile = "D:/file_dir/基于属性的原型实现.docx";
	static String encfile = "D:/file_dir/enc-基于属性的原型实现.docx";
	static String decfile = "D:/file_dir/dec-基于属性的原型实现.docx";

//	static String[] attr = { "baf1", "fim1", "foo" };
//	static String policy = "foo bar fim 2of3";
//	static String attr = "foo bar";
	static String attr = "position:B";
	static String policy = "position:A position:B position:C 1of3";
...
//			+ "ou:idp o:computer mail:student2@sdu.edu.cn title:student";
	static String student_attr = "sn:student2 cn:student2 uid:student2";
		
//	static String student_policy = "sn:student2 cn:te uid:u1 u2 1of2 3of3";
	static String student_policy = "";
	public static void main(String[] args) throws Exception {
//		//加密
//		String policy =po;
//		String attr_str = student_attr;
		long encStartTime = new Date().getTime();
		...
		println("//end to enc");
		long encEndTime = new Date().getTime();
		long encUseTime = (encEndTime-encStartTime)/1000;
		System.out.println("******加密时间："+encUseTime);

		long decStartTime = new Date().getTime();
		...
		println("//end to dec");
		long decEndTime = new Date().getTime();
		long decUseTime = (decEndTime-decStartTime)/1000;
		System.out.println("******解密时间："+decUseTime);
	}
	...
	public static boolean encode(String policy,String fileDir,String fileName,String sourceFilePath) throws Exception {
		try{
			Cpabe test = new Cpabe();
			//加密
			String pubfile = fileDir+"/pub_key";
			String mskfile = fileDir+"/master_key";
			//加密文件存储地址
			String encfile = fileDir+"/enc-"+fileName;
			
			test.setup(pubfile, mskfile);
			//开始加密文件
			test.enc(pubfile, policy, sourceFilePath, encfile);
			//删除源文件
	...
	public static boolean decode (String attr_str,String md5Code,String fileName,String basePath) throws Exception {
		try{
			Cpabe test = new Cpabe();
			File decodeFileDir = new File(basePath+md5Code);
		//	File decodeFileDir = new File("e:/cpabe-file/"+md5Code);
			String prvfile = decodeFileDir.getPath()+"/prv_key";		
			String pubfile = decodeFileDir.getPath()+"/pub_key";
			String mskfile = decodeFileDir.getPath()+"/master_key";	
			//解密文件路径
			String decfile = decodeFileDir.getPath()+"/dec-"+fileName;
			//加密文件路径
			String encfile = decodeFileDir.getPath()+"/enc-"+fileName;
			//生成注册机
